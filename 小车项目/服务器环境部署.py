...
class KerasCategorical(KerasPilot):
    ...
    def __init__(self, resolution=(480, 640), use_smooth=False, *args, **kwargs):
        ...
        super(KerasCategorical, self).__init__(*args, **kwargs)
         # default_categorical(resolution, use_smooth)
...
def default_categorical(resolution, use_smooth):
    ...
    img_in = Input(shape=(resolution[0], resolution[1], 3),
                   name='img_in')
    x = img_in
    # 48个features，3 * 3卷积核，2wx2h步长， relu激活函数
    x = Convolution2D(48, (3, 3), strides=(2, 2), activation='relu')(x)
    # 64个features，5 * 5卷积核，2wx2h步长， relu激活函数
    x = Convolution2D(64, (5, 5), strides=(2, 2), activation='relu')(x)
    # 64个features，5 * 5卷积核，2wx2h步长， relu激活函数
    x = Convolution2D(64, (5, 5), strides=(2, 2), activation='relu')(x)
    # 64个features，3 * 3卷积核，2wx2h步长， relu激活函数
    x = Convolution2D(64, (3, 3), strides=(2, 2), activation='relu')(x)
    # 64个features，3 * 3卷积核，1wx1h步长， relu激活函数
    x = Convolution2D(64, (3, 3), strides=(1, 1), activation='relu')(x)
    # 全连接层
    x = Flatten(name='flattened')(x)
    # 将数据分为100个特征， 激活函数选择relu
    x = Dense(100, activation='relu')(x)
    # 随机关闭20%的神经元（防止过拟合）
    x = Dropout(.2)(x)
    # 将数据分为50个特征， 激活函数选择relu
