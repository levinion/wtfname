...
def default_categorical(resolution, use_smooth):
    ...
    def resnetblock(inputs, residual_path, strides, filters):
        ...
        residual = inputs
        rx = Convolution2D(filters, (3, 3), strides=strides, padding='same', use_bias=False)(inputs)
        rx = BatchNormalization()(rx)
        rx = inrelu(rx)
        rx = Convolution2D(filters, (3, 3), strides=(1, 1), padding='same', use_bias=False)(rx)
        rx = BatchNormalization()(rx)
        if residual_path:
            residual = Convolution2D(filters, (1, 1), strides=(2, 2), padding='same', use_bias=False)(inputs)
    ...
    x = Convolution2D(initial_filters, (3, 3), strides=(1, 1), padding='same', use_bias=False)(x)
